...
export default class Dyno extends Duplex {
  ...
  async start() {
    ...
    if (this.opts.showStatus) {
      let status = `Running ${color.cyan.bold(this.opts.command)} on ${color.app(this.opts.app)}`
      if (this.opts.release) {
        status += ` using release ${color.cyan.bold('v' + this.opts.release)}`
      }

      cli.action.start(status)
    }
  ...
  async _doStart(retries = 2): Promise<HTTP<unknown>> {
    ...
    try {
      ...
      const dyno = await this.heroku.post(this.opts.dyno ? `/apps/${this.opts.app}/dynos/${this.opts.dyno}` : `/apps/${this.opts.app}/dynos`, opts)
