...
export default class RunRelease extends Command {
  static description = 'run a one-off process inside a heroku dyno with a certain release\nShows a notification if the dyno takes more than 20 seconds to start.'

  static examples = [
    '$ heroku run:release --release 42 bash',
    '$ heroku run:release -v 1138 -s hobby -- myscript.sh -a arg1 -s arg2',
  ]
  ...
  async run() {
    ...
    if (!opts.command) {
      throw new Error('Usage: heroku run:release COMMAND\n\nExample: heroku run:release --release 23 bash')
    }
